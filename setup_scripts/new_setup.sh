...
sudo losetup /dev/loop17 foo.img # loop17 – loopback device might be different on system 

#allows grub to install 

#need another loopback device for the actual partition 

sudo losetup /dev/loop24 foo.img -o 1048576 # set the other loopback device to point to the kernel partition (2048*512) 
